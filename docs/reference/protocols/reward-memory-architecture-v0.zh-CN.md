...
`loopx reward-memory ingest-event` 是一条薄的原子编排，而不是新的 memory 产品层。调用方
显式选择 adapter；首个 adapter 是 `issue_fix_maintainer_feedback`。LoopX 不读取或保存原始
comment body，也不会根据关键词自动判断“哪条反馈值得记忆”。模型或调用模块先把材料压缩为
`issue_fix_reward_memory_event_v0`，只保留 source ref、已验证 actor/role、精确
workspace/repository/surface/revision、内容摘要、reasoning 与当前 artifact 证据。
...
不阻塞正常 Issue Fix。`observed_at` 是事件第一次被观察到的不可变时间，retry 必须复用它；
provider target 同时绑定 standing-policy 与 candidate digest，避免策略换版误复用旧激活。
`--execute` 缺省关闭，dry-run 只返回 `planned`。

下一个 patch-planning turn 继续显式调用现有
`run_issue_fix_patch_planning_reward_memory`。模型决定 apply、ignore 或 refute，并复用 Stage 3
application receipt；ingest seam 不增加确定性语义路由或后台 scheduler。
