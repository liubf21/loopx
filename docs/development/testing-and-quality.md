...
| Public-safe decision replay / 公开安全决策回放 | A real failure-shaped quota decision reduces and replays with the same semantics / 真实故障形态经裁剪后语义不漂移 | Regression and control-plane changes / 回归与控制面变更 |
...
4. a public-safe reducer stores only semantic fields, then the replay smoke and
   catalog canary reconstruct the decision without raw state, logs, prompts,
   trajectories, or local paths.

该回归由四层确定性测试保护：数据驱动调度决策表；todo scope 所有权测试；真实 quota
builder 的 fail-closed 集成测试；以及只保留语义字段的公开安全回放与 catalog canary。
