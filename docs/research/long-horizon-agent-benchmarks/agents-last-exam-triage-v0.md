...
This note adds Agents' Last Exam (ALE) to the Goal Harness benchmark planning
...
## Why It Matters For Goal Harness

ALE is more aligned with Goal Harness's long-horizon thesis than a pure coding
...
This makes ALE a good later-stage benchmark for whether Goal Harness improves
...
Terminal-Bench paired pilot yet, because Goal Harness already has Terminal-Bench
...
1. Finish the Terminal-Bench `terminal-bench@2.0/fix-code-vulnerability`
   hardened-codex versus codex-goal-harness paired pilot or record a compact
   runner blocker.
...
4. Define the paired arms with the same hardened install rule:
   `hardened-codex` baseline versus `codex-goal-harness` treatment.
...
Do not compare Goal Harness treatment to public ALE leaderboard entries unless
