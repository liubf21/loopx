...
| `swe-marathon` | `rust-c-compiler` | Goal Harness packet-only observation, host app-server Goal with compact access packet | compact official score `0.0`; ledger upserted under `swe-marathon-rust-c-compiler-treatment-20260621T060729Z`; Goal Harness packet injected, completion marker observed, bridge request count `70`; raw logs, task text, trajectories, and loop-controller trace not read/recorded | completed: packet-only route reached official verifier, but it is not the prompt-driven polling test arm | Treat as route-safety evidence only. The comparable test arm still needs prompt-driven polling with scheduled continuations, blinded official feedback, and public-safe controller trace. |
| `swe-marathon` | `zstd-decoder` | host Codex app-server Goal baseline r2 | compact official score `1.0`; ledger upserted under `swe-marathon-zstd-decoder-baseline-r2-20260621T062826Z`; `goal_harness_inside_case=false`; raw logs, task text, and trajectories not read | completed: baseline pass/control reached official scoring and passed | Treat as the first clean SWE-Marathon native Goal baseline pass. Next comparable work is the same-case Goal Harness prompt-polling test with scheduled continuations and public-safe controller trace. |
| `swe-marathon` | `zstd-decoder` | Goal Harness prompt-driven polling treatment, case-local GH CLI lifecycle | compact official score `0.0`; ledger upserted under `swe-marathon-zstd-decoder-gh-prompt-polling-pr433-20260621T110515Z`; `goal_harness_inside_case=true`; worker bridge verified; case-local GH lifecycle observed with public trace and 12 GH CLI calls; first blocker `harbor_prompt_polling_round_timeout_before_completion`; raw logs, task text, trajectories, and verifier output not read | completed: comparable prompt-driven treatment reached official scoring and regressed from the native Goal baseline | Treat as product-path verified negative/regression evidence, not as uplift. Next SWE work should add public-safe solution/round-timeout counters or run another CPU/no-CUA paired case before changing the main claim table. |
...
- `rust-c-compiler` packet-only Goal Harness observation also reached official
...
- `zstd-decoder` now has the first clean SWE-Marathon native Goal baseline pass
  and a comparable prompt-driven Goal Harness treatment closeout. Baseline r2
  reached official score `1.0`; the GH treatment reached official scoring with
  a verified case-local lifecycle and 12 public-safe GH CLI calls, but returned
  `0.0` with first blocker
  `harbor_prompt_polling_round_timeout_before_completion`. Treat it as
  product-path verified regression evidence, not as a route-connectivity
  blocker or uplift candidate.
...
| P0 | `swe-marathon` | `rust-c-compiler` | official README example and CPU-oriented case; r2 setup/agent/verifier completed at official score `0.0`; packet-only Goal Harness observation also completed at `0.0` | route-safety observation recorded; comparable test requires prompt-driven polling with controller trace before any no-uplift claim. |
| P0 | `swe-marathon` | `zstd-decoder` | no-internet CPU-only case in the first full-suite subset; native Goal baseline r2 passed at official score `1.0`; prompt-driven Goal Harness treatment completed at `0.0` with verified case-local lifecycle | paired evidence recorded as product-path verified regression; next slot should add solution/timeout counters or run another CPU/no-CUA pair before claiming treatment value. |
