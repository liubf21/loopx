...
| `swe-marathon` | `rust-c-compiler` | host Codex app-server Goal completion-aware | compact blocker `swe_marathon_environment_or_image_setup_failure`; no agent execution or verifier result present; follow-up setup preflight found CPU, memory, storage, and Docker root placement satisfy the task envelope | blocked before agent execution, but current capacity gate is clear | Rerun through a prewarmed image/cache path with a larger setup budget; if setup fails again, write a more specific compact setup blocker before counting it against agent quality. |
...
- `rust-c-compiler` is blocked before agent execution by compact
  environment/image setup signals. The follow-up compact setup preflight found
  the current ECS CPU, memory, storage, and Docker root placement satisfy the
  task envelope, so the next SWE step is a prewarmed/larger-budget setup rerun
  rather than a hardware-capacity decision.
...
| P0 | `swe-marathon` | `rust-c-compiler` | official README example and CPU-oriented case; latest run blocked before agent execution, but current setup preflight says ECS capacity is sufficient | rerun through a prewarmed image/cache path with a larger setup budget, then require agent execution or a more specific compact setup blocker. |
