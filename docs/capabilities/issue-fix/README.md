...
| Reward-memory patch planning | `run_issue_fix_patch_planning_reward_memory` | Opt in to one exact reviewed corpus at `issue_fix.patch_planning`; preserve model reasoning inside scope/authority/freshness/conflict guards, require current-artifact verification for influence, and fail open with a compact receipt. |
