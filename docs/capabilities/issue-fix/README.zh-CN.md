...
`reviewer_artifact.summary` surface 时，`reviewer-request` 会独立于二级 sink 是否可用而
解析并预览这次应用。因此，即使单个 PR 的二级通知已暂停，fixer 仍可在零外部写条件下验证
...
| Reward Memory 实验 | `loopx configure-goal --reward-memory-config ... --reward-memory-agent ...`、`loopx reward-memory experiment-status`、`run_issue_fix_patch_planning_reward_memory`、`run_issue_fix_reviewer_artifact_reward_memory` | 默认关闭。允许一个已注册 fixer lane 通过本地 provider binding 试用 `issue_fix.patch_planning`、`reviewer_artifact.summary` 等精确 surface；OpenViking 是当前 provider，不是全局依赖。规划失败时 fail-open；reviewer artifact 在无 sink 时仍可零外部写预览，已配置的二级 reviewer 通知才要求 applied/readback/current-artifact receipt，且不影响 GitHub 主链路。 |
