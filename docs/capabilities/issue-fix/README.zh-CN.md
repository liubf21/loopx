...
当 connected goal 显式为某个 agent 开启 Reward Memory 实验，并包含
`reviewer_artifact.summary` surface 时，已配置的二级通知还必须拿到“当前 PR 已应用且已
回读验证”的 receipt。调用方要用 `--agent-id` 明确自己的注册身份，并提供由模型针对当前
PR 写出的简短中文摘要及应用理由：
...
本地 provider binding。普通 GitHub reviewer request 仍先执行且保持 fail-open；只有显式
配置的二级通知在 receipt 缺失或陈旧时 fail-closed。
...
| Reward Memory 实验 | `loopx configure-goal --reward-memory-config ... --reward-memory-agent ...`、`loopx reward-memory experiment-status`、`run_issue_fix_patch_planning_reward_memory`、`run_issue_fix_reviewer_artifact_reward_memory` | 默认关闭。允许一个已注册 fixer lane 通过本地 provider binding 试用 `issue_fix.patch_planning`、`reviewer_artifact.summary` 等精确 surface；OpenViking 是当前 provider，不是全局依赖。规划失败时 fail-open；已配置的二级 reviewer 通知要求 applied/readback/current-artifact receipt，但不影响 GitHub 主链路。 |
