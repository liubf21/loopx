...
- rolling 默认分支 OpenViking retrieval、显式 reusable-knowledge writeback 与诚实的
...
patch guidance。只有 delivery evidence 提供完整的
`issue_fix_reusable_knowledge_input_v0`，才会写入独立的可复用知识集合：
