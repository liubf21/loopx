...
LoopX 是一个面向长期 agent 工作的本地控制面。它把一次性的静态 goal
...
| LoopX | 维护长期目标的控制面：当前目标、用户决策、agent todo、run history、quota、证据、边界和交接 |

一句话：LoopX 不是 Codex goal 的替代品，而是让 Codex goal、automation
...
LoopX 的产品判断是：强能力 agent-loop 已经存在，问题在于如何把它变成
长期可用、可控、可解释的协作系统。

换句话说，LoopX 想让人的多个 agent 可以持续接力，包括夜间和用户离开
