...
def build_adaptive_task_orchestration_contract(
    *,
    ...
    allowed_domains = {
        domain
        for value in orchestration.get("allowed_domains") or []
    ...
    for item_index, item in enumerate(admitted[1:]):
        if len(selected_children) >= max_children:
            blocked.extend(
                ...
                for deferred_item in admitted[item_index + 1 :]
            )
            break
        conflict = next(
            (
                ...
                if _write_scopes_conflict(existing, item)
            ),
            None,
        )
            ...
            blocked.append(
                {
                    **_blocked_lane(
                    ...
                    "conflicts_with_todo_id": str(
                        conflict.get("todo_id") or ""
                    )
                    or None,
                }
            )
        ...
        selected_children.append(
            _eligible_child_lane(item)
        )
        admitted_write_lanes.append(item)
    ...
    return {
        ...
        "primary_todo_id": str(primary.get("todo_id") or "") or None,
...
def _block_reasons(
    ...
    *,
    ...
    if allowed_domains and task_domain not in allowed_domains:
    ...
    if required_write_scopes and (
        ...
        or any(
            not write_scope_allowed(scope, goal_write_scope)
...
def _blocked_lane(
    ...
    *,
    ...
    return {
        "todo_id": str(item.get("todo_id") or "") or None,
        "task_domain": normalize_todo_task_domain(item.get("task_domain")),
...
def _eligible_child_lane(item: dict[str, Any]) -> dict[str, Any]:
...
def _write_scopes_conflict(
    ...
    left_scopes = normalize_required_write_scopes(left.get("required_write_scopes"))
    right_scopes = normalize_required_write_scopes(right.get("required_write_scopes"))
    if not left_scopes or not right_scopes:
        return False
    left_repository = (
        normalize_todo_task_repository(left.get("task_repository"))
        or "goal_repository"
    )
    right_repository = (
        normalize_todo_task_repository(right.get("task_repository"))
        or "goal_repository"
    )
