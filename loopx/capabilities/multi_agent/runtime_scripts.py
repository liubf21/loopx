...
prompt_target.write_text(
    ...
    "            f'- 当前 frontier: {selected_todo} / {selected_action}',\n"
    "            '- 现在直接运行 `$LOOPX_PANE_A2A_TICK` 一次；这是本 pane 的首轮真实研究动作，不要先做额外 diagnose。',\n"
    "            f'- 若已有 tick 证据: proof_satisfied={str(proof_satisfied).lower()}，仍以当前 LoopX state 为准。',\n"
    "            '- tick 结束后用 3-5 行中文说明：完成了什么、下一步能否自动做、是否需要用户 gate。',\n"
    ...
    "            f'- current frontier: {selected_todo} / {selected_action}',\n"
    "            '- Run `$LOOPX_PANE_A2A_TICK` once now; this is the pane first visible research action, so do not start with an extra diagnose pass.',\n"
    "            f'- if prior tick proof exists: proof_satisfied={str(proof_satisfied).lower()}; current LoopX state remains authoritative.',\n"
    "            '- After the tick, reply with a compact 3-5 line progress/status summary: what changed, what can run next, and whether a user gate is needed.',\n"
...
tick_target.write_text(
    ...
    "    for key in ('selected_todo_id', 'selected_action', 'role_id', 'dev_metric', 'holdout_metric', 'demo_iteration'):\n"
