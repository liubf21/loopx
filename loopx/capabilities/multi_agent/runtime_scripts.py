...
prompt_target.write_text(
    ...
    "            '- 若 tick 没有选中 todo，继续执行上面的首轮动作；必要时再用 `$LOOPX_PANE_LOOPX todo list --goal-id \"$LOOPX_GOAL_ID\"` 找本角色 open todo，不要一上来 diagnose。',\n"
    ...
    "            '- If the tick has no selected todo, continue with the first move above; use `$LOOPX_PANE_LOOPX todo list --goal-id \"$LOOPX_GOAL_ID\"` only when needed, and do not start with diagnose.',\n"
