...
def complete_goal_todo(
    *,
    ...
    with exclusive_file_lock(resolved_state_file):
        ...
        configured_review_agent = side_agent_review_agent_id_from_registry(registry_path, goal_id)
        review_agent = configured_review_agent or primary_agent
        if configured_review_agent:
            review_agent = require_registered_agent_id(
                registry_path=registry_path,
                goal_id=goal_id,
                agent_id=configured_review_agent,
                field="side_agent_review_agent",
            )
        ...
        if side_agent_completion:
            ...
            if not side_agent_self_merged and not next_agent_todo:
                raise ValueError(
                    ...
                    "--next-agent-todo for side-agent review, verification, and merge, "
            ...
            if (
                ...
                and effective_next_claimed_by != review_agent
            ):
                raise ValueError(
                    f"side-agent completion review todo must be claimed_by side_agent_review_agent={review_agent!r}"
                )
            if next_agent_todo and not side_agent_self_merged:
                effective_next_claimed_by = review_agent
                if not next_action_kind:
                    next_action_kind = "side_agent_review" if configured_review_agent else "primary_review"
