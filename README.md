...
| Experimental context learning | Lets named registered agents trial provider-neutral Reward Memory with an ignored local provider binding. It is default-off and explicit; advisory planning fails open, and reviewer-artifact application can be previewed with zero external writes even while secondary sinks are paused. A configured secondary notification may require that verified current-artifact receipt before sending. OpenViking is one provider option, not a global dependency. | `loopx configure-goal --reward-memory-config ... --reward-memory-agent ...`, `loopx reward-memory experiment-status`, [architecture](docs/reference/protocols/reward-memory-architecture-v0.md) |
