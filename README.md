...
| Experimental context learning | Lets named registered agents trial provider-neutral Reward Memory with an ignored project config. Config v1 binds one provider once, assigns explicit compatible corpus sets to module-owned surfaces, and exposes default-off automatic ingest/recall policy; v0 single-corpus configs migrate in memory. No global corpus scan is allowed, failures stay fail-open, and each surface owns its recall profile. OpenViking is one provider option, not a global dependency. | `loopx configure-goal --reward-memory-config ... --reward-memory-agent ...`, `loopx reward-memory experiment-status`, [architecture](docs/reference/protocols/reward-memory-architecture-v0.md) |
