...
| Benchmark run permission policy | `loopx/benchmark_core/run_permissions.py` and `examples/benchmark-run-permission-policy-smoke.py` define `run_permission_policy_v0` plus quota projection for local no-upload benchmark runs. | Do not recreate GH-C23. Useful follow-ups are adapter-by-adapter launch packet adoption and UI display of `goal_boundary.run_permission_policy`. |
...
| GH-C09 | diagnostics | Inspect duplicate run-history index rows with `history inspect-index-duplicates`, then document the current repair path. This is docs-first; code fix can be a follow-up. | `loopx check --scan-path docs/ --scan-path README.md` |
...
| GH-C43 | product docs | Turn the published scenario-gap work into a public-safe `content_ops_surface_v0` fixture or showcase skeleton under `docs/product/` or `docs/showcases/`, using only synthetic/redacted source items, feedback, and publish gates. | `python3 examples/showcase-catalog-smoke.py` plus `loopx check --scan-path docs/product --scan-path docs/showcases` |
| GH-C44 | dashboard | Turn the shipped non-technical operator status model into contributor-facing onboarding copy or a small dashboard fixture: connect the intelligent management surface, project-level reward model, and Project Todo Explorer into plain-language cards such as "what happened", "what is blocked", "what comes next", and "what feedback would change". | `loopx check --scan-path docs/product --scan-path docs/status-data-contract.md --scan-path docs/dashboard-frontend-selection.md`; dashboard fixture if added |
...
| GH-C56 | workflow | Design the first default workflow planner for development-host LoopX usage: model visible TUI, headless runtime, IM/gateway intake, and hybrid handoff as first-class modes, then generate the right scoped workflow from user intent and host capabilities. The planner should cover agent identity, heartbeat/monitor guard, no-spend quiet skip, service/timer or cron fallback when needed, readiness verification, and explicit mode transitions such as TUI-assisted gate -> headless continuation or headless event -> visible TUI escalation. Keep it adapter-neutral and public-safe; do not bake in one chat platform, private host, or project layout. | Design note or fixture plus `loopx check --scan-path docs/ --scan-path CONTRIBUTOR_TASKS.md`; if code is added, include a focused smoke proving the generated workflow carries `--agent-id`, preserves no-spend monitor behavior, and distinguishes TUI, headless, IM/gateway, and hybrid runtime choices |
