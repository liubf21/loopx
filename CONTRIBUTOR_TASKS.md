...
| Visible governance | Quota, scheduler hints, user gates, peer claims, typed continuation, repository policy, and interface budgets already exist in machine contracts. | Show who can act, who must approve, what budget was spent, and how pause/override/terminate decisions map back to LoopX state. |
...
| Todo/status/dashboard | `90c1d0c` shipped the Dashboard Project Todo Explorer, `6f0b88b` taught status to sync todo-index state from rollout events, `24b5ae56` reused run-summary projections for post-handoff runs, and `e33b15d0` fixed monitor-only frontier replan gating. | Do not rebuild the explorer or status/todo wiring. Helpful slices are post-handoff/read-model polish, monitor-versus-advancement projection regressions, clearer demo fixtures, and small browser-safe projection fixes. |
...
| Multi-agent coordination | Earlier slices landed soft todo claims, identity-aware heartbeat prompts, scoped lanes, and self-merge evidence. The peer runtime hard cut removes durable leader/side roles and uses claims, task leases, typed continuation, and task-scoped coordination. | Next public slices are hard per-todo leases, claim transfer/renewal, and operator views that explain peer assignment without inventing hierarchy. |
...
| Peer workspace guard | The contributor-facing contract and `agent_workspace_guard_v1` make `quota should-run` / `spend-slot` fail closed when a repository-writing peer violates the selected task/repository isolation policy. | Do not duplicate the base guard. Helpful follow-ups are hard lease conflict checks and real UI rendering of task-scoped workspace repair. |
...
| Auto-research productization | Earlier slices proved visible launcher, worker-loop, role profile, and evidence append paths; `a7fd8067`, `8891dd3a`, `bb8b4df4`, and `006ca81f` then tightened startup defaults, added visible worker-hook validation, and fixed quiet-completion frontier behavior. | Keep the mainline focused on the generic multi-agent runner, pane-local LoopX tick, and a small action/evidence kernel. Good follow-ups are operator walkthroughs, synthetic fixtures, and narrowly scoped status/readiness polish, not a return to bespoke board or acceptance-packet layers. |
| Release/canary readiness | `fa631504` made `loopx canary premerge` show in-flight progress and added a dedicated validation-gate smoke. | Do not invent one-off release checklists outside the shipped canary surface. Helpful follow-ups are contributor-safe docs, fixture coverage, and small route-label/readiness polish around the existing premerge contract. |
...
| SkillsBench runtime boundary | `56e1b532`, `2c60a594`, and `afc7cf73` tightened app-server first-action timeout handling, pre-agent setup attribution, and no-proxy egress support in the public SkillsBench workflow; `2413b447`, `8e23eae2`, `9fd517bc`, `b82a011b`, `71f4f4b8`, and `a46a98bb` then made the Codex Goal route canonical, hardened TUI startup, clarified pre-bridge status, hardened the bridge first-action prompt, and split build-stall timeout coverage into a focused smoke. | Do not bypass the run-permission or preflight path. Useful follow-ups are reducer/display polish, public-safe docs, and focused route/attribution or build-stall/first-action smokes that clarify launch-ready versus setup-blocked cases without publishing raw benchmark artifacts. |
...
| GH-C64 | release docs | Add a contributor-safe `loopx canary premerge` walkthrough that explains progress/readiness output, the dedicated validation-gate smoke, and how to stop before maintainer-only rollout or local automation state. | `python3 examples/canary/premerge-validation-gate-smoke.py` and `loopx check --scan-path docs/ --scan-path CONTRIBUTOR_TASKS.md` |
...
| GH-C40 | benchmark | Extend the benchmark developer workflow product path after the ECS tooling landed: add ALE compact readiness/blocker commands, document Terminal-Bench ECS launch hygiene plus the canonical Codex Goal / SkillsBench route, and deepen no-upload reducer, preflight, build-stall timeout, and app-goal setup-tail docs so contributors can distinguish launch-ready cases from verifier-bootstrap, first-action, or setup blockers without launching private benchmark jobs or copying raw artifacts. | `python3 examples/benchmark-developer-workflow-doc-smoke.py`, `python3 examples/benchmark-ecs-developer-tooling-smoke.py`, `python3 examples/skillsbench-build-stall-timeout-smoke.py`, and a new focused smoke if code is added |
...
| GH-C43 | showcase | Turn the shipped auto-research worker-loop and visible supervisor path into a contributor-safe operator walkthrough or showcase fixture under `docs/product/` or `docs/showcases/`, using only synthetic/redacted lane state, artifacts, takeover gates, visible worker-hook behavior, and quiet-completion steps that match the current Codex CLI flow. | `python3 examples/showcase-catalog-smoke.py`, `python3 examples/auto-research-worker-loop-smoke.py`, `python3 examples/auto-research-visible-worker-hook-smoke.py`, and `loopx check --scan-path docs/product --scan-path docs/showcases` |
