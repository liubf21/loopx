...
| Goal channel projection | `4b5b46c` defined `goal_channel_projection_v0`; the first read-only fixture now builds compact channel snapshots from status, quota, run history, todos, gates, artifacts, and soft claims while emitting `source_warnings` for raw/private inputs. | Next slices should render this projection in the dashboard/frontstage or wire it into a read-only CLI/status export without making the channel writable truth. |
