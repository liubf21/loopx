...
| --- | --- | --- |
| Decision and material productization | `a8d69355` assembled revision-bound Decision Context evidence; `c53e77e8`, `ebbc9b7a`, and `f02e2f8c` added lossless material preparation, owner-gated apply, and readable ranking projections; `64f4a7a8` added public capability guides. | Add one synthetic cross-capability walkthrough or missing negative fixture. Keep activation default-off and leave private source adapters, cursor state, raw material, and apply authority outside public fixtures. |
...
| CLI/runtime boundaries | `c788088b` added visible Codex App goals over SSH, `ce3836cd` added managed project-skill delivery, and `393bac6d` exposed recoverable LoopX Turn execution stages. | Add provider-neutral fake-host examples, compact stage/receipt rendering, or one missing failure-mode fixture. Preserve independent validation and keep recurring loops outside `run-once`. |
| Status, quota, and monitors | `95badcda` extracted registry-boundary projection, `e7d8940f` extracted benchmark run projection, and `4bc8e1dc` propagated capability-gated material envelopes. | Add focused parity fixtures or compact operator explanations. Keep hot-path output bounded, cold-path detail available, and unavailable capabilities visible without adding another projection source. |
| Benchmark boundary | `e7d8940f` moved benchmark status shaping into its read-model home; `69ee55a1` added typed setup-failure receipts; recent SkillsBench repairs preserve progress and terminal attribution across timeout, preflight, proxy, and runtime-rebind failures. | Extend another adapter through the shared lifecycle/read-model seams or add synthetic failure attribution. Keep live comparisons coordination-required and raw task text, logs, trajectories, verifier tails, credentials, uploads, and local paths out of public fixtures. |
| Validation and change quality | `e2d2025d` added exact-scope qualification; `6453ffb6`, `ebc0b39c`, `fe9860f5`, and `064c3d79` made it simplify-first, repository-oracle aware, sparse, and receipt-enforced. | Add independently derived rules or one missing repository-oracle adapter. Discovered task names must remain review inputs, not auto-executed commands, and any diff change must invalidate the old receipt. |
| Release and install | v0.2.13 is the current tagged version. `docs/product/release-readiness.md` still stops at v0.2.6 even though v0.2.7-v0.2.13 are tagged; release snapshots, canaries, and installed-runtime activation remain distinct boundaries. | Close the v0.2.7-v0.2.13 history gap, then improve contributor-safe update recovery and failure attribution without adding a parallel release checklist. |
...
| GH-C60 | workflow | Add one focused fake-fixture parity slice across Codex App heartbeat, Codex CLI TUI, LoopX Turn, Claude Code loop, OpenCode, shell worker, HTTP webhook, and worker bridge. Treat the shipped OpenCode selector/catalog identity as the reference, then cover one missing authoritative action, scoped identity, runtime-owned cadence, no-spend transition, workspace repair, or private-boundary case. | `python3 examples/host-mode-plan-smoke.py`, `python3 examples/project/host-mode-plan-cli-smoke.py`, focused OpenCode bridge tests, `python3 -m pytest -q tests/test_loopx_turn_transaction.py`, and `loopx check --scan-path docs/runtime-connector-catalog.md --scan-path docs/reference/protocols/host-mode-plan-v0.md --scan-path CONTRIBUTOR_TASKS.md` |
