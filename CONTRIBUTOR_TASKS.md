...
| Conversational commands | `global_manager_command_v0` defines read-only commands such as `/loop-global-summary`, `/loop-global-gates`, and `/loop-global-todos`. | Implement one canonical command at a time with a public-safe smoke and no alias sprawl. Unknown commands should fail closed with help. |
...
| --- | --- | --- |
| CLI/runtime | `536ee63` extracted the ML experiment command group into `loopx/cli_commands/ml_experiment.py` and added `regression/cli-command-module-contract.py`. | Do not redesign the module seam from scratch. Useful follow-ups are one more low-risk command-group extraction, better module-contract coverage, or CLI help/compatibility polish. |
...
| Auto-research productization | `291f9b1`, `f45ab4d`, `5d80bf1`, and `88b898b` added the auto-research board projection packet, acceptance packet, visible launcher path, and public role profiles. | The public seams are landed. Useful follow-ups are operator walkthrough copy, dashboard fixture polish, and small validation slices that keep the board, launcher, and role profile surfaces aligned. |
...
| GH-C53 | docs | Add a contributor-safe release/readiness explainer for default vs canary installs, promotion-readiness smokes, and when to stop before local `.loopx` or maintainer-only automation state. | `loopx check --scan-path README.md --scan-path docs/ --scan-path CONTRIBUTOR_TASKS.md` |
| GH-C57 | docs | Add a contributor-safe heartbeat repair guide for identity-aware automation: when to regenerate `loopx heartbeat-prompt --thin --agent-id --agent-scope`, how stale unscoped prompts fail, and how to verify the repaired prompt without touching `.local` state. | `loopx check --scan-path README.md --scan-path docs/ --scan-path CONTRIBUTOR_TASKS.md` |
...
| GH-C40 | benchmark | Extend the benchmark developer workflow product path after the ECS tooling landed: add ALE compact readiness/blocker commands, document the Terminal-Bench ECS launch hygiene and native Codex Goal launcher path, and deepen SkillsBench no-upload reducers/preflight docs so contributors can distinguish launch-ready cases from verifier-bootstrap blockers without launching private benchmark jobs or copying raw artifacts. | `python3 examples/benchmark-developer-workflow-doc-smoke.py`, `python3 examples/benchmark-ecs-developer-tooling-smoke.py`, and a new focused smoke if code is added |
...
| GH-C43 | showcase | Turn the shipped auto-research board packet, acceptance packet, and visible launcher path into a contributor-safe operator walkthrough or showcase fixture under `docs/product/` or `docs/showcases/`, using only synthetic/redacted lane state, artifacts, and takeover gates. | `python3 examples/showcase-catalog-smoke.py`, `python3 examples/auto-research-demo-acceptance-smoke.py`, and `loopx check --scan-path docs/product --scan-path docs/showcases` |
| GH-C44 | dashboard | Turn the shipped non-technical operator status model and new auto-research board packet into a management-surface fixture: map LoopX kernel objects to user concepts such as work item, owner, decision, evidence, budget, risk, next action, and takeover point. Keep `goal_channel_projection_v0` / `task_graph_projection_v0` read-only and use raw machine fields only as drill-down evidence. | `loopx check --scan-path docs/product --scan-path docs/status-data-contract.md --scan-path docs/dashboard-frontend-selection.md`; dashboard fixture if added |
...
| GH-C49 | dashboard | Polish the shipped `/frontstage` goal-channel board: improve visual acceptance coverage, local demo fixture clarity, and operator onboarding affordances while keeping `attention_queue.items[].goal_channel_projection` read-only browser data. | `npm run smoke:frontstage-route`, `npm run smoke:frontstage-browser`, and `loopx check --scan-path apps/dashboard --scan-path docs/dashboard-frontend-selection.md` |
...
| GH-C60 | workflow | Add focused smoke coverage for the runtime connector catalog rows: Codex App heartbeat, Codex CLI TUI, Claude Code loop, shell worker, HTTP webhook, and worker bridge. Keep the tests adapter-neutral and assert scoped identity, scheduler hints, no-spend cadence/final-check behavior, and private-boundary stripping. | Focused smoke(s) plus `loopx check --scan-path docs/runtime-connector-catalog.md --scan-path CONTRIBUTOR_TASKS.md` |
...
| GH-C61 | cli | Implement the next canonical global manager command after `/loop-global-summary`: choose one of `/loop-global-gates`, `/loop-global-todos`, or `/loop-goal-summary`, keep it read-only, source it from compact status/quota/todo/run-history projections, and make unknown aliases fail closed with help instead of broad dumps. | Focused command smoke plus `python3 examples/global-manager-command-protocol-smoke.py` and `loopx check --scan-path docs/reference/protocols/global-manager-command-v0.md --scan-path CONTRIBUTOR_TASKS.md` |
