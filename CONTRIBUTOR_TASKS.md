...
| GH-C40 | benchmark | Improve the benchmark developer workflow product path for the cloud Codex default route: add one stable command or fixture that checks cloud-host readiness, emits a no-upload runner batch or precise blocker, and updates compact docs without private logs. | `python3 examples/benchmark-developer-workflow-doc-smoke.py` plus focused runner/adapter smoke |
