...
| Goal channel projection | `4b5b46c` defined `goal_channel_projection_v0`; `6d37e3f` added the compact projection builder; the current fixture renders it into static read-only frontstage HTML with semantic panels, soft-claim visibility, source warnings, and no write controls. | Next slices should wire this projection into the real dashboard data feed or expose it through a read-only CLI/status export without making the channel writable truth. |
...
| GH-C49 | dashboard | Wire `goal_channel_projection_v0` into the real dashboard feed or a read-only CLI/status export, reusing the frontstage fixture semantics instead of adding write authority to the channel view. | `python3 examples/goal-channel-frontstage-fixture-smoke.py` plus dashboard or CLI export smoke |
