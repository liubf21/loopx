...
| GH-C40 | benchmark | Improve the benchmark developer workflow product path: add one stable command or fixture that takes a split-control readiness payload, emits a runner batch or precise blocker, and updates compact docs without private logs. | `python3 examples/benchmark-developer-workflow-doc-smoke.py` plus focused runner/adapter smoke |
