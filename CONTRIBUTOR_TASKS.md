...
| Side-agent worktree guard | `47841a9` and `afc6aa7` made `quota should-run` / `spend-slot` fail closed when a side agent runs from the registered primary checkout, plus a focused smoke. | Do not duplicate the base guard. Helpful follow-ups are dashboard/review-packet projection, clearer examples, and claim-aware selection regressions. |
...
| GH-C45 | coordination | Extend the side-agent worktree guard into contributor-facing examples, dashboard/review-packet copy, or claim-aware selection regression so side agents can see why a primary-checkout or primary-owned todo is not their next action. | Focused smoke or docs check around `workspace_guard`, `claimed_by`, and side-agent handoff surfaces |
