...
| Conversational commands | `global_manager_command_v0` defines read-only commands such as `/loopx-global-summary` and `/loopx-global-gates` (both implemented), plus `/loopx-global-todos` and `/loopx-global-risks`; legacy `/loop-global-*` forms are only migration aliases. | Implement `/loopx-global-todos` or `/loopx-global-risks` next, one canonical command at a time with a public-safe smoke and no alias sprawl. Unknown commands should fail closed with help. |
...
| GH-C82 | validation / runtime | Add a public negative fixture for typed Turn settlement: a legacy plan without a typed settlement must fail closed with an actionable upgrade path, and a fresh typed settlement receipt must round-trip through the CLI without leaking provider fields, invoking a model, sleeping, or spending quota twice. | `python3 -m pytest -q tests/control_plane/test_quota_settlement.py tests/control_plane/test_quota_settlement_cli.py tests/test_loopx_turn_driver.py tests/test_loopx_turn_executor.py` and `loopx check --scan-path docs/reference/protocols/loopx-turn-v0.md --scan-path CONTRIBUTOR_TASKS.md` |
